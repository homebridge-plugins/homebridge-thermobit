...
class GoveePlatform {
  ...
  initialiseDevice (device) {
    ...
    try {
      ...
      if (this.consts.models.rgb.includes(device.model)) {
        ...
        instance =
          this.deviceConf[device.device] && this.deviceConf[device.device].showAs
            ? this.deviceConf[device.device].showAs
            : this.consts.defaultValues.showAs
        if (instance === 'default' || instance === 'purifier') {
          instance = 'outlet'
        }
        instance += '-double'
      ...
      // Temporarily override the debugging of the heater/humidifier whilst in dev
      if (['heater', 'humidifier'].includes(instance)) {
        accessory.context.enableLogging = true
        accessory.context.enableDebugLogging = true
      }
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          if (parsed && parsed.bleName) {
            ...
            accessory.context.bleAddress =
              deviceConf && deviceConf.customAddress
    ...
    return await this.queue.add(async () => {
      ...
      try {
        ...
        switch (params.cmd) {
          ...
          case 'brightness': {
            ...
            */
            ...
            if (!this.consts.noAWSBrightnessColour.includes(accessory.context.gvModel)) {
              data.awsParams = {
                cmd: 'brightness',
          ...
          case 'colorTem': {
            ...
            */
            ...
            if (!['H6072', 'H6199'].includes(accessory.context.gvModel)) {
              data.awsParams = {
                cmd: 'colorTem',
                data: {
                  color: {
        ...
        try {
          ...
          if (
            accessory.context.enableDebugLogging &&
            (accessory.context.useAWSControl || err.message !== this.lang.notAvailable)
          ) {
            const eText = this.funcs.parseError(err, [
        ...
        // API doesn't support dual outlets and scenes yet so return now
        if (
          (['stateDual', 'scene'].includes(params.cmd) && data.awsParams) ||
