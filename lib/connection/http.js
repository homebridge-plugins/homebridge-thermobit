...
module.exports = class connectionHTTP {
  ...
  async getDevice () {
    try {
      ...
      if (err.code && this.consts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
        this.log.warn('%s [HTTP - getDevice()] - %s.', this.lang.httpRetry, err.code)
        await this.funcs.sleep(30000)
  ...
  async setDeviceSchedule (newData) {
    try {
      ...
      const unitNumber = parseInt(this.password)
      const data = [
        {
          Day: '1',
          Selected: true,
      ...
      this.log('%s.', this.lang.scheduleUpdated)
    } catch (err) {
      const eText = this.funcs.parseError(err)
      this.log.warn('%s %s.', this.lang.scheduleFailed, eText)
    }
  }
}
