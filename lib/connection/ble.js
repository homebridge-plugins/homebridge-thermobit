...
const btClient = require('@homebridge/noble')
...
module.exports = class BluetoothLED {
  ...
  async updateDevice (accessory, params) {
    ...
    if (this.stateChange !== 'poweredOn') {
      throw new Error(this.lang.btWrongState)
    }
    ...
    setTimeout(
      () => (doIt = false),
      this.consts.models.rgbBT.includes(accessory.context.gvModel) ? 3000 : 2000
    )
    ...
    while (true) {
      // Timeout if taking over 2 seconds for wifi models and 3 seconds for only bluetooth models
      if (!doIt) {
