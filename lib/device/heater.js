...
export default class {
    ...
    try {
      ...
      if (!this.disableDeviceLogging) {
        this.log(
          ...
          this.cacheState ? platformLang.labelMan : platformLang.labelSch,
        );
      }
    } catch (err) {
      // Catch any errors and let the platform display them
      const eText = platformFuncs.parseError(err);
      this.log.warn('[%s] %s %s.', this.accessory.displayName, platformLang.updateFail, eText);
      setTimeout(() => {
  ...
  async internalTargetUpdate(value) {
    try {
      ...
      // Log the change
      if (!this.disableDeviceLogging) {
        this.log('[%s] %s [%s°C].', this.accessory.displayName, platformLang.curTarg, this.cacheTarg);
      }
    } catch (err) {
      // Catch any errors and let the platform display them
      const eText = platformFuncs.parseError(err);
  ...
  externalUpdate(params) {
    ...
    if (platformFuncs.hasProperty(params, 'Mode')) {
    ...
    if (this.cacheState && platformFuncs.hasProperty(params, 'IsOn')) {
      if (params.IsOn !== this.cacheHeat) {
        ...
        if (!this.disableDeviceLogging) {
          this.log(
            '[%s] %s [%s].',
            this.accessory.displayName,
            platformLang.curHeat,
            this.cacheHeat ? platformLang.labelHeating : platformLang.labelIdle,
          );
        }
      }
    }
    ...
    if (platformFuncs.hasProperty(params, 'CurrentTemp')) {
      if (params.CurrentTemp !== this.cacheTemp) {
        ...
        // Log the change
        if (!this.disableDeviceLogging) {
          this.log('[%s] %s [%s°C].', this.accessory.displayName, platformLang.curTemp, this.cacheTemp);
        }
      }
    }

    // Check to see if the provided target temperature is different from the cached state
    if (this.cacheState && platformFuncs.hasProperty(params, 'SetTemp')) {
